...
| `AUTH_SHOW_CODE` | **temporary.** `1` returns the sign-in code to the browser and shows it on the page; email, if configured, is sent in the background and never blocks the request. Nobody's inbox is verified while set. Use only until SMTP works. To turn it off, set it to `0` (or delete the line; `casino restart` passes an explicit `0` when the key is absent, because pm2 never drops a variable on its own) and `casino restart`. |
